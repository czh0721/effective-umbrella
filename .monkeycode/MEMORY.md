...
- Instructions:
  ...
  - 定价规则（2026-09-18 与用户确认）：1 元 = 10 念念币、1 念念币 = 100 积分、每轮扣 20 积分（即 ¥0.02/轮）；按 60% 毛利率、优化后每轮成本约 ¥0.008 反推。套餐结构为 `store.PACKAGE_TIERS`：30/90/365 天各轻享/标准/尊享三档（尊享附赠蒸馏券 1/2/3 张），不提供入门体验包。一次性迁移 `_migrate_package_tiers`（meta 标记 `package_tiers_v1`，下架 `标准包`/`尊享包`）与 `_migrate_retire_entry_package`（`packages_retired_v1`，下架 `体验包`）按名处理；后台改价后不会被重启覆盖。调整套餐/扣费/赠送时必须做成本-收入核算。
