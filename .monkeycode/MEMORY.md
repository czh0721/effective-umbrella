...
- Instructions:
  ...
  - 定价规则（2026-09-18 与用户确认）：1 元 = 10 念念币、1 念念币 = 100 积分、每轮扣 20 积分（即 ¥0.02/轮）；按 60% 毛利率、优化后每轮成本约 ¥0.008 反推。套餐结构为 `store.PACKAGE_TIERS`：30/90/365 天各轻享/标准/尊享三档（尊享附赠蒸馏券 1/2/3 张），另有入门 `体验包`（10 币 / 1000 积分 / 30 天）。一次性迁移 `_migrate_package_tiers`（meta 标记 `package_tiers_v1`）与 `_migrate_entry_package`（`entry_package_v2`）按名 upsert 档位并下架旧 `标准包`/`尊享包`；后台改价后不会被重启覆盖。调整套餐/扣费/赠送时必须做成本-收入核算。
  - 成本实测方法（2026-09-18）：生产用 `/opt/nian/venv/bin/python scripts/cost_probe.py` 借平台模型跑真实调用（只读配置、不写业务表、不打印密钥，可用 `COST_PROBE_CHARS`/`COST_PROBE_CALLS` 调参）。实测 `deepseek-chat` 稳定前缀缓存命中率约 98%，二次及以后每轮成本 <¥0.001，会话首轮冷启动 ¥0.005–0.014（随提示长度），平均低于 ¥0.008，60% 毛利成立。生产 systemd 服务 ExecStart 为 `/opt/nian/venv/bin/python -m ex_persona.cli web`，EnvironmentFile `/opt/nian/.env`。
