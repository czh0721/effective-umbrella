...
# 已下线：入门体验包。保留名称用于一次性下架迁移，不再作为默认套餐发放。
RETIRED_PACKAGE_NAMES = ("体验包",)
...
def _migrate_retire_entry_package(conn: sqlite3.Connection) -> None:
    """一次性下架入门体验包（幂等，只执行一次）。

    用户确认不再需要体验包。这里只把同名套餐置为下架，保留数据与历史订单；用
    ``schema_meta`` 标记保证只执行一次。
    """
    if _meta_get(conn, "packages_retired_v1") == "1":
        return
    now = utcnow()
    placeholders = ", ".join("?" * len(RETIRED_PACKAGE_NAMES))
    conn.execute(
        f"UPDATE credit_packages SET active = 0, updated_at = ?"
        f" WHERE name IN ({placeholders}) AND active = 1",
